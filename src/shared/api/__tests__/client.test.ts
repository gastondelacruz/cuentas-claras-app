...
import { getRefreshToken, setRefreshToken } from "../tokenStorage";
...
describe("api client", () => {
	...
	it("marks the session unverified on likely unverified-email 403 without logging out", async () => {
		...
		mock
			.onPost("/groups")
			.reply(403, { message: "Email verification required" });

		await expect(client.post("/groups", {})).rejects.toMatchObject({
			response: { status: 403 },
		});
	...
	it("does not mark the session unverified for unrelated already-verified 403 responses", async () => {
		...
		mock
			.onPost("/auth/email-verification/verify")
			.reply(403, { message: "Email already verified" });

		await expect(
			client.post("/auth/email-verification/verify", { token: "used-token" }),
	...
	it("reads the refresh response envelope, persists the rotated token, and retries without losing the login", async () => {
		useAuthStore
			.getState()
			.setSession({ id: "1", email: "a@b.com" }, "old-token");
	...
	it("does not refresh or retry a request more than once", async () => {
		await setRefreshToken("old-refresh-token");
		mock.onGet("/groups").replyOnce(401).onGet("/groups").replyOnce(401);
		mock.onPost("/auth/refresh").reply(200, {
			data: { accessToken: "new-token", refreshToken: "rotated-refresh-token" },
		});
		...
		expect(
			mock.history.post.filter((request) => request.url === "/auth/refresh"),
		).toHaveLength(1);
	...
	it("logs out without calling refresh when no refresh token exists", async () => {
		useAuthStore
			.getState()
			.setSession({ id: "1", email: "a@b.com" }, "old-token");
		...
		await expect(client.get("/groups")).rejects.toThrow(
	...
	it("preserves the session and refresh token when refresh fails transiently", async () => {
		useAuthStore
			.getState()
			.setSession({ id: "1", email: "a@b.com" }, "old-token");
		await setRefreshToken("old-refresh-token");
		...
		const unsubscribe = onAuthLogout(logoutListener);
		mock.onGet("/groups").replyOnce(401);
		mock.onPost("/auth/refresh").reply(503);

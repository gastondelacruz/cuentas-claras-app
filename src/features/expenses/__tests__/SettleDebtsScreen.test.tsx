...
import { getGroup, getGroupBalances, getGroupSettlements, recordGroupSettlement } from '../../groups/api/groupsApi';
...
const currentUserBalance = { memberId: 'm1', displayName: 'Vos', balance: 0, currency: 'ARS', isCurrentUser: true };
const anaBalance = { memberId: 'm-ana', displayName: 'Ana', balance: 50, currency: 'ARS' };
...
describe('SettleDebtsScreen', () => {
  ...
  beforeEach(() => {
    ...
    jest.mocked(useRoute).mockReturnValue({ params: { groupId: 'g1' } } as never);
    mockGetGroup.mockResolvedValue({ id: 'g1', members: [] });
    mockGetGroupSettlements.mockResolvedValue({ settlements: [] });
    mockRecordGroupSettlement.mockResolvedValue({ payment: mockPayment, balances: [] });
  });
  ...
  it('shows a "Saldar" button on user-facing settlement rows and calls the mutation on press', async () => {
    ...
    mockGetGroupSettlements.mockResolvedValue({
      settlements: [
        { fromMemberId: 'm-ana', fromMemberName: 'Ana', toMemberId: 'm1', toMemberName: 'Vos', amount: 50, currency: 'ARS' },
      ],
    });
    ...
    await waitFor(() => expect(mockRecordGroupSettlement).toHaveBeenCalledTimes(1));
    await waitFor(() => expect(button).not.toBeDisabled());
    expect(mockRecordGroupSettlement).toHaveBeenCalledWith(
      'g1',
      expect.objectContaining({ fromMemberId: 'm-ana', toMemberId: 'm1', amount: 50 }),
    );
  });
  ...
  it('renders the balances summary and settlement rows from real data', async () => {
    mockGetGroupBalances.mockResolvedValue({
      balances: [
        ...
        { memberId: 'm-ana', displayName: 'ana@example.com', balance: -50, currency: 'ARS' },
      ],
    });
    mockGetGroupSettlements.mockResolvedValue({
      settlements: [
        { fromMemberId: 'm-ana', fromMemberName: 'ana@example.com', toMemberId: 'm1', toMemberName: 'Vos', amount: 50, currency: 'ARS' },
      ],
    });
  ...
  it('renders the you-owe direction with user-facing copy and settles from the current user', async () => {
    ...
    mockGetGroupSettlements.mockResolvedValue({
      settlements: [
        { fromMemberId: 'm1', fromMemberName: 'Vos', toMemberId: 'm-ana', toMemberName: 'Ana', amount: 40, currency: 'ARS' },
      ],
    });
    ...
    expect(mockRecordGroupSettlement).toHaveBeenCalledWith(
      'g1',
      expect.objectContaining({ fromMemberId: 'm1', toMemberId: 'm-ana', amount: 40 }),
    );
  });
});

...
describe('groupsApi.createGroup', () => {
  ...
  it('maps FOOD category to other type', async () => {
    ...
    expect(mockPost).toHaveBeenCalledWith('/groups', expect.objectContaining({ type: 'other' }));
  });
...
describe('groupsApi.getGroup', () => {
  ...
  it('throws when the response is missing id', async () => {
    ...
    await expect(getGroup('g1')).rejects.toThrow('API response does not match contract');
  });
...
describe('groupsApi.updateGroup', () => {
  ...
  it('calls PATCH /groups/:id with the payload and returns parsed data', async () => {
    ...
    mockPatch.mockResolvedValueOnce({ data: { data: { ...validGroupResponse, name: 'Viaje a Bariloche' } } });
  ...
  it('throws when the response does not match the contract', async () => {
    ...
    await expect(updateGroup('g1', { name: 'Invalid' })).rejects.toThrow('API response does not match contract');
  });
});
...
describe('groupsApi.deleteGroup', () => {
  ...
  it('calls DELETE /groups/:id and returns parsed data', async () => {
    mockDelete.mockResolvedValueOnce({ data: { data: { ...validGroupResponse, archivedAt: '2024-05-20T00:00:00.000Z' } } });
  ...
  it('throws when the response is missing id', async () => {
    ...
    await expect(deleteGroup('g1')).rejects.toThrow('API response does not match contract');
  });
});
...
describe('groupsApi.getGroupBalances', () => {
  ...
  it('throws when the response does not match the contract', async () => {
    mockGet.mockResolvedValueOnce({ data: { data: { balances: [{ id: 'm1' }] } } });

    await expect(getGroupBalances('g1')).rejects.toThrow('API response does not match contract');
  });
});
...
describe('groupsApi.getGroupSettlements', () => {
  ...
  it('calls GET /groups/:id/settlements and returns parsed data', async () => {
    ...
    const data = {
      settlements: [
        { fromMemberId: 'm1', fromMemberName: 'Vos', toMemberId: 'm2', toMemberName: 'Ana', amount: 30, currency: 'ARS' },
      ],
    };
    mockGet.mockResolvedValueOnce({ data: { data } });
  ...
  it('throws when the response does not match the contract', async () => {
    mockGet.mockResolvedValueOnce({ data: { data: { settlements: [{ id: 's1' }] } } });

    await expect(getGroupSettlements('g1')).rejects.toThrow('API response does not match contract');
  });
});
...
describe('groupsApi.recordGroupSettlement', () => {
  ...
  it('calls POST /groups/:id/settlements and returns parsed data', async () => {
    const input = { fromMemberId: 'm1', toMemberId: 'm2', amount: 30, currency: 'ARS', paidAt: '2026-06-26T12:00:00.000Z' };
    const data = {
      payment: {
        id: 's1', groupId: 'g1',
        fromMember: { id: 'm1', displayName: 'Vos' },
        toMember: { id: 'm2', displayName: 'Ana' },
        amount: 30, currency: 'ARS', paidAt: '2026-06-26T12:00:00.000Z', notes: null, createdAt: '2026-06-26T12:00:00.000Z',
      },
      balances: [{ memberId: 'm1', displayName: 'Vos', balance: 0, currency: 'ARS' }],
    };
  ...
  it('throws when the response does not match the contract', async () => {
    mockPost.mockResolvedValueOnce({ data: { data: { payment: { id: 's1' } } } });

    await expect(recordGroupSettlement('g1', {
      fromMemberId: 'm1',
      toMemberId: 'm2',
      amount: 30,
      currency: 'ARS',

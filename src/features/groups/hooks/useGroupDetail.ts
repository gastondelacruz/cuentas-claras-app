...
function buildTimeLabel(dateValue: string, now = new Date()): string {
	...
	const diffDays = Math.round(
		(toStartOfDay(now) - toStartOfDay(date)) / oneDay,
	);
...
function mapExpenseListItemToGroupExpense(
...
export function useGroupDetail(groupId?: string): UseGroupDetailResult {
	...
	const isFetching =
		isDetailFetching || isBalancesFetching || isExpensesLoading;

	if (!groupId || !protectedDataEnabled || !groupDetail) {
		return {
			group: null,
			memberBalances: [],
	...
	const rawBalances = protectedDataEnabled
	...
	const recentExpenses = protectedDataEnabled
		? expenses.map(mapExpenseListItemToGroupExpense)
		: [];

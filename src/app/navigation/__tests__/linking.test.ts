...
describe('root deep-linking config', () => {
  ...
  it('parses email verification custom-scheme links with token params', () => {
    const state = getStateFromCuentasClarasURL('cuentasclaras://verify-email?token=abc');

    expect(state?.routes[0]).toEqual({ name: 'VerifyEmail', params: { token: 'abc' }, path: 'verify-email?token=abc' });
  });

  it('parses group invitation custom-scheme links with token params', () => {
    const state = getStateFromCuentasClarasURL('cuentasclaras://group-invitations/accept?token=abc');

    expect(state?.routes[0]).toEqual({ name: 'AcceptGroupInvitation', params: { token: 'abc' }, path: 'group-invitations/accept?token=abc' });
  });
});

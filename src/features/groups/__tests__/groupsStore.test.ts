...
describe('useGroupsStore', () => {
  ...
  it('deletes a created group by id and keeps a deletion tombstone', () => {
    const createdGroup = useGroupsStore.getState().createGroup({
      name: 'Road Trip 2026',
      category: 'TRAVEL',
      image: { type: 'default', uri: null },
    ...
    useGroupsStore.getState().deleteGroup(createdGroup.id);

    expect(useGroupsStore.getState().groups.find((group) => group.id === createdGroup.id)).toBeUndefined();
    expect(useGroupsStore.getState().deletedGroupIds).toContain(createdGroup.id);
  });
});

...
describe("personalTransactionSchema", () => {
	it("preserves an expense kind returned by the API", () => {
		const result = personalTransactionSchema.parse({
			id: "ptx-fixed",
			type: "expense",
			expenseKind: "fixed",
			amount: 50000,
		...
		expect(result).toEqual(expect.objectContaining({ expenseKind: "fixed" }));
	});

	it("tolerates a temporarily missing expense kind during backend rollout", () => {
		const result = personalTransactionSchema.parse({
			id: "ptx-legacy",
			type: "expense",
			amount: 50000,
		...
		expect(result.expenseKind).toBeUndefined();
	});
});

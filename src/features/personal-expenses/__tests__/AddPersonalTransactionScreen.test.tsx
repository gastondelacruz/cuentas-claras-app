...
describe('AddPersonalTransactionScreen', () => {
  beforeEach(() => {
    ...
    jest.mocked(useRoute).mockReturnValue({ params: { type: 'expense' } } as never);
  ...
  it('renders the add expense form with Stitch categories, account, and date chips', () => {
  ...
  it('submits an expense transaction payload and navigates back', async () => {
    ...
    fireEvent.changeText(screen.getByTestId('personal-transaction-amount-input'), '12.500');
  ...
  it('renders income categories and submit copy for the income route', () => {
    jest.mocked(useRoute).mockReturnValue({ params: { type: 'income' } } as never);
  ...
  it('submits an income transaction payload', async () => {
    ...
    fireEvent.changeText(screen.getByTestId('personal-transaction-amount-input'), '876.371');
    ...
    await waitFor(() => {
      expect(mockCreatePersonalTransaction).toHaveBeenCalledWith(
        expect.objectContaining({ type: 'income', amount: 876371, category: 'Salario' }),
        expect.any(Object),
      );
    });
  });
  ...
  it('tapping "hoy" chip submits occurredAt equal to today at noon UTC', async () => {
    ...
    fireEvent.changeText(screen.getByTestId('personal-transaction-amount-input'), '100');
  ...
  it('deselects the previous category when a different one is tapped', () => {
    ...
    expect(screen.getByTestId('personal-category-Ocio').props.accessibilityState).toMatchObject({
      selected: true,
    });
    expect(screen.getByTestId('personal-category-Salud').props.accessibilityState).toMatchObject({
      selected: false,
  ...
  it('makes the calendar button tappable and no longer disabled', () => {
    ...
    expect(calendarButton.props.accessibilityState).not.toMatchObject({ disabled: true });
  ...
  it('shows the expense note placeholder on the GASTOS tab', () => {
    ...
    expect(screen.getByTestId('personal-note-input').props.placeholder).toBe('¿En qué gastaste?');
  });
  ...
  it('shows the income note placeholder after switching to the INGRESOS tab', () => {
    ...
    expect(screen.getByTestId('personal-note-input').props.placeholder).toBe('¿De qué es este ingreso?');
  });
  ...
  it('submits the trimmed note in the create payload when provided', async () => {
    ...
    fireEvent.changeText(screen.getByTestId('personal-transaction-amount-input'), '100');
    fireEvent.changeText(screen.getByTestId('personal-note-input'), '  Farmacia  ');

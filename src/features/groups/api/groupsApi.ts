...
export async function createGroup(
  ...
  const response = await client.post<{ data: CreateGroupResponse }>(
    "/groups",
    input,
  );
...
export async function updateGroup(
  id: string,
  data: Partial<GroupDetailDto>,
): Promise<GroupDetailDto> {
  const response = await client.patch<{ data: GroupDetailDto }>(
    `/groups/${id}`,
    data,
  );
  return parseOrThrow(groupDetailSchema, response.data.data);
...
export async function deleteGroup(id: string): Promise<GroupDetailDto> {
  const response = await client.delete<{ data: GroupDetailDto }>(
    `/groups/${id}`,
  );
  return parseOrThrow(groupDetailSchema, response.data.data);
}
...
export async function getGroupBalances(id: string): Promise<GroupBalancesDto> {
  const response = await client.get<{ data: GroupBalancesDto }>(
    `/groups/${id}/balances`,
  );
...
export async function getGroupSettlements(
  ...
  const response = await client.get<{ data: GroupSettlementsResponseDto }>(
...
export async function recordGroupSettlement(
  ...
  const response = await client.post<{ data: RecordSettlementResponseDto }>(
    `/groups/${id}/settlements`,
    input,
  );

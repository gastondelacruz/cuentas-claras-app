    ...
    <ScreenContainer>
      ...
      >
          ...
          {FORM_TABS.map((tab) => {
            ...
            return (
              <Pressable
                ...
                accessibilityLabel={
                  tab.label === 'GASTOS' ? 'Añadir gasto personal' : 'Añadir ingreso personal'
                }
        ...
        >
          ...
          >
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
              <Text style={{ fontSize: 13, color: GRAY, fontWeight: '500' }}>ARS</Text>
        ...
        <View style={{ paddingHorizontal: 16, paddingTop: 16 }}>
          <Text style={{ fontSize: 12, fontWeight: '600', color: GRAY, marginBottom: 4 }}>
            Descripción
          </Text>
          <TextInput
        ...
        >
          <Text style={{ fontSize: 12, fontWeight: '600', color: GRAY }}>Cuenta</Text>
          <Pressable accessibilityRole="button" accessibilityLabel="Seleccionar cuenta">
            <Text style={{ fontSize: 14, color: PRIMARY, fontWeight: '500' }}>Pesos</Text>
          </Pressable>
        </View>
            ...
            {dateChips.map((chip) => {
              ...
              return (
                <Pressable
                  key={chip.id}
                  accessibilityRole="button"
                  accessibilityLabel={chip.subLabel ? `Fecha ${chip.label} ${chip.subLabel}` : `Fecha ${chip.label}`}
                  accessibilityState={{ selected }}
                  onPress={() => setSelectedDateId(chip.id)}
                  testID={chip.id === 'custom' ? 'date-chip-custom' : `personal-date-${chip.id}`}
                  style={{

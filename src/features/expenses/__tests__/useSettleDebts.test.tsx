...
describe("useSettleDebts", () => {
	...
	it("does not fetch settlements when email is not verified", () => {
		useAuthStore
			.getState()
			.setEmailVerification({ verified: false, verifiedAt: null });

		const { result, unmount } = renderHook(() => useSettleDebts("g1"), {
			wrapper: Wrapper,
		});
		...
		expect(mockGetGroupSettlements).not.toHaveBeenCalled();
	...
	it("returns a zero summary when balances are all settled", async () => {
		...
		const { result } = renderHook(() => useSettleDebts("g1"), {
			wrapper: Wrapper,
		});
	...
	it("maps backend settlements to who-owes-whom rows relative to the current user", async () => {
		mockGetGroupBalances.mockResolvedValue({
			balances: [{ ...currentUser, balance: 30 }],
		});
			...
			settlements: [
				{
					fromMemberId: "m-ana",
					fromMemberName: "Ana",
					toMemberId: "m-current",
					toMemberName: "Vos",
					amount: 50,
					currency: "ARS",
				},
				{
					fromMemberId: "m-current",
					fromMemberName: "Vos",
					toMemberId: "m-beto",
					toMemberName: "Beto",
					amount: 20,

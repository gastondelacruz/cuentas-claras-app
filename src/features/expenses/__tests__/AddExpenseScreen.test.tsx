...
import { getGroup } from "../../groups/api/groupsApi";
...
const MEMBERS_PRIMARY = [
	...
	{
		id: "invite-0-alex@example.com",
		displayName: "alex@example.com",
		isCurrentUser: false,
	},
	{
		id: "invite-1-sarah@example.com",
...
const MEMBERS_SECONDARY = [
	{ id: "current-user", displayName: "Vos", isCurrentUser: true },
	{
		id: "invite-0-diego@example.com",
		displayName: "diego@example.com",
		isCurrentUser: false,
	},
	{
		id: "invite-1-lucia@example.com",
		displayName: "lucia@example.com",
		isCurrentUser: false,
	},
];
...
describe("AddExpenseScreen", () => {
	beforeEach(() => {
		...
		mockGetGroup.mockImplementation(async (groupId: string) => {
			...
			throw new Error(`Unexpected group detail request: ${groupId}`);
		});
	...
	it("does not submit when the amount is zero or less", async () => {
		...
		fireEvent.changeText(
			screen.getByTestId("expense-description-input"),
			"Cena",
		);
		...
		await waitFor(() => {
			expect(mockCreateExpense).toHaveBeenCalledWith(PRIMARY_GROUP_ID, {
				...
				participantMemberIds: [
					"current-user",
					"invite-0-alex@example.com",
					"invite-1-sarah@example.com",
				],
				splitType: "equal",
				category: "FOOD",
				notes: null,
	...
	it("uses backend group-member ids from a newly created group detail cache when creating an expense", async () => {
		...
		jest
			.mocked(useRoute)
			.mockReturnValue({ params: { groupId: newGroupId } } as ReturnType<
				typeof useRoute
			>);

		render(<AddExpenseScreen />, { wrapper: Wrapper });

		expect(screen.getByTestId("expense-paidby-field")).toHaveTextContent(
			"Gaston",
		);
		...
		await waitFor(() => {
			expect(mockCreateExpense).toHaveBeenCalledWith(
				...
				expect.objectContaining({
					...
					participantMemberIds: [
						"backend-current-member-id",
						"backend-friend-member-id",
					],
				}),
			);
		});
	...
	it("preselects the group received from navigation params", () => {
		jest.mocked(useRoute).mockReturnValue({
			params: { groupId: SECONDARY_GROUP_ID },
		} as ReturnType<typeof useRoute>);

		render(<AddExpenseScreen />, { wrapper: Wrapper });

		expect(screen.getByTestId("expense-group-field")).toHaveTextContent(
			"Departamento",
		);
		expect(screen.getByText("diego@example.com")).toBeTruthy();
	});
	...
	it("returns to the existing group detail when saving from that group", async () => {
		jest
			.mocked(useRoute)
			.mockReturnValue({ params: { groupId: PRIMARY_GROUP_ID } } as ReturnType<
				typeof useRoute
			>);
	...
	it("pops to the selected group detail when changing groups before saving", async () => {
		...
		fireEvent.press(screen.getByTestId("expense-group-field"));
		fireEvent.press(
			within(screen.getByTestId("expense-group-modal")).getByText(
				"Departamento",
			),
		);
		fireEvent.changeText(screen.getByTestId("expense-amount-input"), "900");
		...
		expect(navigationMock.popTo).toHaveBeenCalledWith("GroupDetail", {
	...
	it("shows error when selected group is not found in the groups list", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText(
					"El grupo seleccionado ya no está disponible. Elegí otro grupo.",
				),
			).toBeTruthy();
		});
		expect(mockCreateExpense).not.toHaveBeenCalled();
	...
	it("keeps at least one participant selected", () => {
		...
		fireEvent.press(
			screen.getByTestId("expense-participant-invite-0-alex@example.com"),
		);
		fireEvent.press(
			screen.getByTestId("expense-participant-invite-1-sarah@example.com"),
		);
	...
	it('disables "Seleccionar todos" when everyone is already selected', () => {
		...
		expect(selectAllButton.props.accessibilityState).toMatchObject({
			disabled: true,
		});
		...
		expect(
			screen.getByTestId("expense-select-all").props.accessibilityState,
		).toMatchObject({
			disabled: false,
		});
	...
	describe("edit mode", () => {
		...
		it("prefills the form from the selected expense", () => {
			...
			expect(
				screen.getByTestId("expense-category-FOOD").props.accessibilityState,
			).toMatchObject({
				selected: true,
			});
		});
		...
		it("does not create a new expense if edit data has not loaded yet", async () => {
			...
			expect(
				screen.getByText(
					"Estamos cargando el gasto. Esperá unos segundos e intentá de nuevo.",
				),
			).toBeTruthy();
		});
		...
		it("calls the update expense API and navigates back on save", async () => {
			...
			await waitFor(() => {
				...
				expectExpenseMutationInvalidations(
					invalidateSpy,
					PRIMARY_GROUP_ID,
					editableExpense.id,
				);
		...
		it("keeps the group fixed while editing", () => {
			...
			expect(
				screen.getByTestId("expense-group-field").props.accessibilityState,
			).toMatchObject({
				disabled: true,
			});
		});
